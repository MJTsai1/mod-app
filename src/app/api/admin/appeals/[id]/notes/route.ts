...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  const { data: appeal } = await supabase.from("ban_appeals").select("id").eq("id", id).maybeSingle();
  if (!appeal) return NextResponse.json({ error: "Not found." }, { status: 404 });

  const { data: note, error } = await supabase
    .from("case_notes")
    .insert({ entity_type: "appeal", entity_id: id, staff_id: session.staff.id, note: parsed.data.note })
