...
import { updateSupportRequestSchema } from "@/lib/validation/support";
import { resolveClaimAction } from "@/lib/claim";
import { logActivity } from "@/lib/activityLog";
import { notifyDiscordOfSupportStatusChange } from "@/lib/discord";
...
export async function GET(_request: Request, { params }: RouteParams) {
  ...
  if (error) {
    console.error("Failed to load support request:", error.message);
...
export async function PATCH(request: Request, { params }: RouteParams) {
  ...
  const parsed = updateSupportRequestSchema.safeParse(rawBody);
  if (!parsed.success) {
  ...
  if (error) {
    console.error("Failed to update support request:", error.message);
  ...
  if (parsed.data.status && parsed.data.status !== current.status) {
    notifyDiscordOfSupportStatusChange(data, current.status).catch(() => {});
  }
